        ...
        {MONTHS.map(m => {
          ...
          return (
            <div key={m} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
              ...
              <div style={{ fontSize: 7, color: m === selectedMonth ? c : '#475569', fontWeight: m === selectedMonth ? 700 : 400 }}>{m.slice(0, 1)}</div>
            </div>
          );
        })}
      </div>
                  ...
                  <thead>
                    <tr style={{ background: '#0a0f1e' }}>
                      ...
                      {Object.keys(collectorTotals).map(c => (
                        <th key={c} style={{ padding: '8px 10px', textAlign: 'right', color: COLLECTOR_COLORS[c] || '#94a3b8', fontSize: 11 }}>{c.split('/')[0]}</th>
                      ))}
                  ...
                  <tfoot>
                    <tr style={{ borderTop: '2px solid #1e293b', background: '#0a0f1e' }}>
                      ...
                      {Object.entries(collectorTotals).map(([c, months]) => {
                        ...
                        return <td key={c} style={{ padding: '9px 10px', textAlign: 'right', color: COLLECTOR_COLORS[c] || '#94a3b8', fontWeight: 700 }}>₹{fmtNum(t)}</td>;
                      })}
                      <td style={{ padding: '9px 12px', textAlign: 'right', color: pgColor, fontWeight: 800 }}>
                        ₹{fmtNum(Object.values(collectorTotals).reduce((s, m) => s + Object.values(m).reduce((ss, v) => ss + v, 0), 0))}
                      </td>
                    </tr>
                  </tfoot>
                </table>
