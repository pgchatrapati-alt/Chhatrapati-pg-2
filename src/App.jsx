...
export default function App() {
  ...
  // FIX 8: editTab default = 'payments' (open payments first on tenant click)
  const [editingTenant,setEditingTenant] = useState(null);
  const [editTab,setEditTab] = useState('payments'); // 'payments' | 'info'
  ...
  // FIX 8: open modal with payments tab by default
  function openEdit(tenant, defaultTab='payments'){
  ...
  // FIX 6+7: Call/WhatsApp always visible
  ...
  // ── Computed total paid for a tenant ─────────────────────
  function getTotalPaid(t){
    return MONTHS.reduce((s,m)=>s+(parseFloat(t.monthly?.[m]?.amount)||0),0);
  }
  ...
  function EditModal(){
    ...
    const isActive=!editForm.dateLeaving||new Date(editForm.dateLeaving)>=new Date();
    const totalPaid=getTotalPaid(editingTenant);
    const depositOk=editForm.deposit&&editForm.deposit!==''&&editForm.deposit!=='0';

    return(
      <div className="mb" style={{position:'fixed',inset:0,background:'rgba(0,0,0,.88)',zIndex:300,display:'flex',alignItems:'flex-end',justifyContent:'center',backdropFilter:'blur(4px)'}} onClick={e=>{if(e.target===e.currentTarget)setEditingTenant(null);}}>
        <div className="mc" style={{background:'linear-gradient(180deg,#0d1626,#070d1a)',width:'100%',maxWidth:640,borderRadius:'20px 20px 0 0',maxHeight:'93vh',overflowY:'auto',border:'1px solid #1e2d45',borderBottom:'none'}}>

          {/* Drag handle */}
          <div style={{width:36,height:4,background:'#1e2d45',borderRadius:4,margin:'14px auto 0'}}/>

          {/* Header */}
          <div style={{padding:'16px 20px 0'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:6}}>
              <div>
                <div style={{fontWeight:800,fontSize:22,letterSpacing:'-.01em'}}>{editingTenant.name}</div>
                <div style={{fontSize:12,color:'#475569',marginTop:3,display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>
                  <span>{pg}</span>
                  <span style={{width:5,height:5,borderRadius:'50%',background:isActive?'#22c55e':'#64748b',display:'inline-block',boxShadow:isActive?'0 0 6px #22c55e':'none'}}/>
                  <span style={{color:isActive?'#22c55e':'#64748b'}}>{isActive?'Active':'Inactive'}</span>
                  <span>• Joined {fmtDate(editForm.dateJoining)}</span>
                </div>
                {/* FIX 7: Contact always shown */}
                {editForm.contact&&<div style={{fontSize:12,color:'#94a3b8',marginTop:4}}>📞 {editForm.contact}</div>}
              </div>
              <div style={{display:'flex',gap:6,alignItems:'center'}}>
                ...
                <button className="bp" onClick={()=>setEditingTenant(null)} style={{background:'#1e2d45',border:'1px solid #334155',color:'#94a3b8',padding:'7px 11px',borderRadius:9,cursor:'pointer',fontSize:14}}>✕</button>
              </div>
            </div>

            {/* FIX 6+7: Call & WhatsApp — ALWAYS visible prominently */}
            {editForm.contact&&(
              <div style={{display:'flex',gap:10,margin:'14px 0 16px'}}>
                <button className="bp ch" onClick={e=>doCall(editForm.contact,e)}
                  style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',gap:7,background:'linear-gradient(135deg,#22c55e22,#16a34a22)',border:'1.5px solid #22c55e55',color:'#22c55e',padding:'11px',borderRadius:12,cursor:'pointer',fontSize:14,fontWeight:700,fontFamily:'Outfit,sans-serif'}}>
                  📞 Call
                </button>
                <button className="bp ch" onClick={e=>doWhatsApp(editForm.contact,editingTenant.name,e)}
                  style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',gap:7,background:'linear-gradient(135deg,#25d36622,#128c4422)',border:'1.5px solid #25d36655',color:'#25d366',padding:'11px',borderRadius:12,cursor:'pointer',fontSize:14,fontWeight:700,fontFamily:'Outfit,sans-serif'}}>
                  💬 WhatsApp
                </button>
              </div>
            )}
            ...
            {/* Stats row */}
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:1,background:'#1e2d45',borderRadius:14,overflow:'hidden',marginBottom:16}}>
              {[
                {label:'Total Paid',val:`₹${totalPaid.toLocaleString()}`,color:'#38bdf8'},
                {label:'Monthly Rent',val:`₹${(parseFloat(editForm.rent)||0).toLocaleString()}`,color:'#94a3b8'},
                {label:'Deposit',val:depositOk?`₹${editForm.deposit}`:'⚠ Pending',color:depositOk?'#22c55e':'#f59e0b'},
              ].map(s=>(
                <div key={s.label} style={{background:'#0a1525',padding:'12px 14px',textAlign:'center'}}>
                  <div style={{fontSize:10,color:'#475569',fontWeight:600,letterSpacing:'.05em',marginBottom:4}}>{s.label}</div>
                  <div style={{fontSize:16,fontWeight:800,color:s.color,fontFamily:'JetBrains Mono,monospace'}}>{s.val}</div>
                </div>
              ))}
            </div>

            {/* FIX 8: Tab switcher — Payments opens first */}
            <div style={{display:'flex',gap:2,background:'#070d1a',borderRadius:12,padding:3,marginBottom:16}}>
              {[['payments','💳 Payments'],['info','ℹ️ Info']].map(([tab,label])=>(
                <button key={tab} className="bp" onClick={()=>setEditTab(tab)}
                  style={{flex:1,padding:'9px',borderRadius:9,border:'none',background:editTab===tab?pgC:'transparent',color:editTab===tab?'#fff':'#64748b',fontWeight:700,fontSize:13,cursor:'pointer',fontFamily:'Outfit,sans-serif',transition:'all .2s'}}>
                  {label}
                </button>
              ))}
            </div>
          ...
          <div style={{padding:'0 20px 28px'}}>
            ...
            <div style={{display:'flex',gap:8,marginTop:20,paddingTop:16,borderTop:'1px solid #1e2d45'}}>
              {isAdmin
                ?<button className="bp ch" onClick={saveEdit} style={{flex:1,background:`linear-gradient(135deg,${pgC},${pgC}cc)`,border:'none',color:'#fff',padding:'13px',borderRadius:12,cursor:'pointer',fontWeight:700,fontSize:15,fontFamily:'Outfit,sans-serif',boxShadow:`0 4px 20px ${pgC}44`}}>💾 Save + Auto Sync</button>
      ...
      <div style={{minHeight:'100vh',background:'#050810',fontFamily:'Outfit,sans-serif'}}>
        ...
        <div style={{padding:'14px 16px',maxWidth:900,margin:'0 auto'}}>
          ...
          {pgTs.map((t,i)=>{
            ...
            return(
              <div key={t.name+t.dateJoining} className="ch fu row" onClick={()=>{setSelectedPG(activePGDetail);openEdit(t,'payments');}}
                style={{background:'linear-gradient(135deg,#0d1626,#0a0f1e)',borderRadius:14,padding:'14px 16px',border:`1px solid ${status.isPending&&isActive?'#ef444422':'#1e2d45'}`,cursor:'pointer',marginBottom:8,animationDelay:`${i*40}ms`}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                  <div style={{flex:1}}>
                    ...
                    {/* FIX 6+7: Always show call/whatsapp */}
                    {t.contact&&(
                      <div style={{display:'flex',gap:6,marginTop:8}} onClick={e=>e.stopPropagation()}>
                        <button className="bp" onClick={e=>doCall(t.contact,e)} style={{background:'#22c55e18',border:'1px solid #22c55e33',color:'#22c55e',padding:'4px 12px',borderRadius:20,cursor:'pointer',fontSize:11,fontWeight:700,fontFamily:'Outfit,sans-serif'}}>📞 Call</button>
      ...
      <main style={{padding:'16px',maxWidth:900,margin:'0 auto'}}>
        ...
        {view==='tenants'&&(<>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:8}}>
            {filtered.map((t,i)=>{
              ...
              return(
                <div key={t.name+t.dateJoining} className="ch fu" style={{background:'linear-gradient(135deg,#0d1626,#0a0f1e)',borderRadius:14,padding:'14px 16px',border:`1px solid ${status.isPending&&isActive?'#ef444422':'#1e2d45'}`,animationDelay:`${i*30}ms`}}>
                  {/* Top row — click opens modal */}
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',cursor:'pointer'}} onClick={()=>openEdit(t,'payments')}>
                    <div style={{flex:1}}>
                  ...
                  {/* FIX 6+7: Always visible Call/WhatsApp */}
                  {t.contact&&(
                    <div style={{display:'flex',gap:6,marginTop:10,paddingTop:10,borderTop:'1px solid #1e2d4566'}}>
                      <button className="bp" onClick={e=>doCall(t.contact,e)} style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',gap:5,background:'#22c55e14',border:'1px solid #22c55e33',color:'#22c55e',padding:'7px',borderRadius:9,cursor:'pointer',fontSize:12,fontWeight:700,fontFamily:'Outfit,sans-serif'}}>📞 Call</button>
                      <button className="bp" onClick={e=>doWhatsApp(t.contact,t.name,e)} style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',gap:5,background:'#25d36614',border:'1px solid #25d36633',color:'#25d366',padding:'7px',borderRadius:9,cursor:'pointer',fontSize:12,fontWeight:700,fontFamily:'Outfit,sans-serif'}}>💬 WhatsApp</button>
                    </div>
                  )}
                </div>
              );
        ...
        {view==='monthly'&&(<>
          <div style={{display:'flex',gap:5,marginBottom:14,overflowX:'auto',paddingBottom:2}}>
            {MONTHS.map(m=>(
              <button key={m} className="bp" onClick={()=>setSelectedMonth(m)} style={{padding:'4px 12px',borderRadius:20,border:`1px solid ${selectedMonth===m?'transparent':'#1e2d45'}`,background:selectedMonth===m?pgColor:'#111827',color:selectedMonth===m?'#fff':'#64748b',cursor:'pointer',fontSize:11,fontWeight:600,fontFamily:'Outfit,sans-serif',whiteSpace:'nowrap',transition:'all .18s'}}>{m}</button>
            ))}
          </div>
          <div style={{background:'linear-gradient(135deg,#0d1626,#0a0f1e)',borderRadius:16,border:'1px solid #1e2d45',overflow:'hidden'}}>
            ...
            {filtered.filter(t=>!t.dateLeaving||new Date(t.dateLeaving)>=new Date()).map(t=>{
              const status=getRentStatus(t,selectedMonth);
              return(<div key={t.name} className="row" onClick={()=>openEdit(t,'payments')} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'12px 18px',borderBottom:'1px solid #0d1626',cursor:'pointer'}}>
        ...
        {view==='collectors'&&(<>
          <div style={{display:'flex',gap:5,marginBottom:14,overflowX:'auto',paddingBottom:2}}>
            {MONTHS.map(m=>(
              <button key={m} className="bp" onClick={()=>setSelectedMonth(m)} style={{padding:'4px 10px',borderRadius:20,border:`1px solid ${selectedMonth===m?'transparent':'#1e2d45'}`,background:selectedMonth===m?pgColor:'#111827',color:selectedMonth===m?'#fff':'#64748b',cursor:'pointer',fontSize:11,fontWeight:600,fontFamily:'Outfit,sans-serif',whiteSpace:'nowrap',transition:'all .18s'}}>{m.slice(0,3)}</button>
            ))}
          </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(150px,1fr))',gap:10,marginBottom:20}}>
            {Object.entries(collectorStats).map(([collector,stats],i)=>{
              const c=COLLECTOR_COLORS[collector]||'#94a3b8';const monthAmt=stats.months[selectedMonth]||0;
          ...
          <div style={{background:'linear-gradient(135deg,#0d1626,#0a0f1e)',borderRadius:16,border:'1px solid #1e2d45',overflow:'hidden'}}>
            <div style={{overflowX:'auto'}}>
              <table style={{width:'100%',borderCollapse:'collapse',fontSize:12,fontFamily:'Outfit,sans-serif'}}>
                ...
                <tbody>
                  {MONTHS.map(m=>{
                    const rowTotal=Object.values(collectorStats).reduce((s,st)=>s+(st.months[m]||0),0);const isSel=m===selectedMonth;
                    return(<tr key={m} style={{borderTop:'1px solid #1e2d45',background:isSel?'#ffffff05':'transparent',cursor:'pointer'}} onClick={()=>setSelectedMonth(m)}>
                      ...
                      {Object.entries(collectorStats).map(([c,st])=>(
                        <td key={c} style={{padding:'10px 16px',textAlign:'right',color:st.months[m]?'#f1f5f9':'#1e2d45',fontFamily:'JetBrains Mono,monospace',fontWeight:600}}>{st.months[m]?`₹${st.months[m].toLocaleString()}`:'—'}</td>
                      ))}
                      <td style={{padding:'10px 16px',textAlign:'right',color:rowTotal?pgColor:'#1e2d45',fontWeight:800,fontFamily:'JetBrains Mono,monospace'}}>{rowTotal?`₹${rowTotal.toLocaleString()}`:'—'}</td>
                    </tr>);
                  })}
                </tbody>
              </table>
